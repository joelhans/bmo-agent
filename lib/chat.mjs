...
function buildSystemPrompt() {
  ...
  const parts = [];
  parts.push(`You are bmo — a fast, pragmatic, and self-improving coding agent. Use tools when needed.\n\nAvailable tools: ${toolNames}`);
  try {
    let notesPath = '';
    if (fs.existsSync('AGENTS.md')) notesPath = path.resolve('AGENTS.md');
    else {
      const bundled = path.join(getToolsDir(), 'BMO_AGENTS.md');
