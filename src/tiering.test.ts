...
describe("selectInitialTier", () => {
	test("returns reasoning for simple messages (new default)", () => {
		expect(selectInitialTier({ userMessage: "fix the typo in main.ts", lastResponseWasError: false })).toBe("reasoning");
		expect(selectInitialTier({ userMessage: "add a button to the header", lastResponseWasError: false })).toBe("reasoning");
